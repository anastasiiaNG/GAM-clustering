load("~/R-studio/nclust/reflink.rda")
library(igraph)
library(rUtils)
# for correct working of "GAM-clustering per se" slot:
...
############################
# NCmisc::list.functions.in.file("csourse.R", alphabetic = TRUE)
...
# heinz2mc.solver
# loadClusters
# makeANetworkWithScore
# makeAtomGraph
# makeAtomNetworkWithCorTest
# makeENetworkWithCorTest
# makeENetworkWithDist
# makeENetworkWithScore
# makeNetwork
# makeNetworkWithCorTest
# makeNetworkWithDist
# mwcsize
# processModule
# replaceNA
...
processModule <- function(module, dir, s=34, name=NULL) {
  ...
  #     E(module)$label <- ""
  if (is.null(igraph::V(module)$logPval)) {
    igraph::V(module)$logPval <- -5
  }
  if (is.null(V(module)$log2FC)) {
    igraph::V(module)$log2FC <- NA
  }
...
makeAtomGraph <- function(network,
  ...
  if (!is.null(met.to.filter)) {
    nodes.to.del <- igraph::V(g)[metabolite %in% met.to.filter]
    if (length(nodes.to.del) == 0) {
      warning("Found no metabolites to mask")
    } else {
      g <- delete_vertices(g, v = igraph::V(g)[metabolite %in% met.to.filter])
    }
  }
  g
}
makeENetworkWithScore <- function(score, base, network) {
  ...
  suppressWarnings(es.re <- GAM::makeExperimentSet(network, gene.de=fake.gene.de,
                                              reactions.as.edges=T, plot=F))
  subnet.scored <- es.re$subnet
  #V(subnet.scored)$score <- -0.1
  igraph::V(subnet.scored)$score <- -0.01
  rxn.vertices <- igraph::V(subnet.scored)[nodeType == "rxn"]
...
sgmwcs.batchSolver2 <- function(sgmwcs, nthreads = 1, timeLimit = -1,
  ...
  function(networks) {
    ...
    instances <- lapply(seq_len(N), function(i) {
      ...
      if (!score.nodes) {
        igraph::V(network)$score <- 0
      }
      if (!score.edges) {
        igraph::E(network)$score <- 0
      }
...
makeNetwork <- function(gene.de, met.de=NULL, largest.component=TRUE) {
  ...
  res <- delete.vertices(atom.graph, igraph::V(atom.graph)[KEGG %in% kegg.db$mets2mask]$name)
...
collapseAtoms2 <- function(m) {
  t <- data.frame(v=seq_along(igraph::V(m)), met=gsub("_.*$", "", igraph::V(m)$name))
  t <- data.frame(v=igraph::V(m)$name, met=gsub("_.*$", "", igraph::V(m)$name), stringsAsFactors=F)
...
addEdges <- function(m, net, es) {
  m.temp <- induced.subgraph(net, igraph::V(m)$name)
...
mwcsize <- function(g) {
  ...
  et[, from.weight := pmax(-igraph::V(g)[et$from]$score, 0)+1e-3]
  et[, to.weight := pmax(-igraph::V(g)[et$to]$score, 0)+1e-3]
  et <- et[score > 0, ]
  ...
  igraph::V(g)[ds$v]$score <- igraph::V(g)[ds$v]$score + ds$d
  igraph::E(g)$origEdge <- igraph::E(g)
  igraph::E(g)[score > 0]$score <- 0


  neg.edges <- igraph::E(g)[score < 0]
  neg.edges.table <- get.edgelist(g)[neg.edges, ]
  neg.edges.names <- sprintf("%s_%s_%s", neg.edges.table[,1], neg.edges.table[,2], neg.edges)
  igraph::V(g)$wasEdge <- FALSE
...
demwcsize <- function(m, g) {
  igraph::E(g)$origEdge <- igraph::E(g)
  orig.vertices <- igraph::V(g)[name %in% igraph::V(m)$name]
  edges.to.add <- unique(c(
    igraph::E(induced.subgraph(g, orig.vertices))[score >= 0]$origEdge,
    na.omit(c(igraph::V(m)$origEdge, igraph::E(m)$origEdge))))
...
makeNetworkWithDist <- function(gene.dist.to.cluster, base, network) {
  ...
  es.rn <- GAM::makeExperimentSet(network, gene.de=fake.gene.de, reactions.as.edges=F, plot=F)
...
makeENetworkWithDist <- function(gene.dist.to.cluster, base, network) {
  ...
  es.re <- GAM::makeExperimentSet(network, gene.de=fake.gene.de, reactions.as.edges=T, plot=F)
  subnet.scored <- es.re$subnet
...
makeENetworkWithScore <- function(score, base, network) {
  ...
  suppressWarnings(es.re <- GAM::makeExperimentSet(network, gene.de=fake.gene.de, reactions.as.edges=T, plot=F))
  subnet.scored <- es.re$subnet

  #V(subnet.scored)$score <- -0.1
  igraph::V(subnet.scored)$score <- 0
  rxn.vertices <- igraph::V(subnet.scored)[nodeType == "rxn"]
  #     E(subnet.scored)$score <-
  #         -(1 - pmax(1 - E(subnet.scored)$dist.closest, 0)) +
  #         base
