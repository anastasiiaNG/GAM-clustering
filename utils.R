extractMetabolicGenes <- function(gene.exprs = Biobase::exprs(es.top12k),
  ...
  rownames(gene.exprs) <- entrezIDs

  # GAM
  exprsMetab <- gene.exprs[rownames(gene.exprs) %in% network$rxn2gene$gene, ]
  # gatom
  ...
  print(sprintf("There are %d metabolic genes in the current dataset", dim(exprsMetab)[1]))

  return(list(
    exprsMetab = exprsMetab,
    network = network
    ))
}

findParameters <- function(exprsMetab = exprsMetab,
                           work.dir = work.dir){
  ### finding k
  ...
  gene.cor <- cor(t(exprs), use="pairwise.complete.obs")
  gene.cor.dist <- as.dist(1 - gene.cor)

  print("Calculating elbow plot for 1:64 clusters. It can take several minutes...")
  y = factoextra::fviz_nbclust(x = exprs,
                               FUNcluster = cluster::pam,
                               diss = gene.cor.dist,
                               method = "wss",
                               k.max = 14) # replace by 64 after testing

  y +
    ggplot2::labs(subtitle = "Elbow method") +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, vjust = 0.5, hjust=1))
  ggplot2::ggsave(paste0(work.dir, "elbowPlot.svg"), width = 9, height = 3)
  save(y, file=paste0(work.dir, "elbowData.Rda"))

  print("Calculating the 50% slope point relative to the steepest slope of the curve...")
  # https://rdrr.io/github/agentlans/KneeArrower/src/R/KneeArrow.R
  sugK <- KneeArrower::findCutoff(as.numeric(y$data$clusters), y$data$y)

  gKneeArrower <- sugK$x
  gK <- if(gKneeArrower >= 32) sugK$x else 32


  ### finding base
  gene.pam <- cluster::pam(gene.cor.dist, k=gK)
  base <- round(mean(gene.pam$clusinfo[, 3]), digits = 1)

  return(list(
    gKneeArrower = gKneeArrower,
    gK = gK,
    base = base
  ))
}
...
preClustering <- function(gene.exprs = exprsMetab$exprsMetab,
...
gamClustering <- function(gene.exprs = preCluster$gene.exprs,
  ...
  while (T) {
    while (T) {
      ...
      curCenters <- rev$centers.pos ### again TRY rev$centers.all (1)
...
getGraphs <- function(curRev = gamCluster$curRev$modules,
  ...
  for (i in seq_along(curRev)) {
    ...
    processModule(module = m1, dir = work.dir, name = sprintf("%s.%s", "m", i)) # add selecting seed for graph layout tuning: s=1
...
getGeneTables <- function(curRev = gamCluster$curRev,
  ...
  for (i in seq_along(curRev$modules)) {
    ...
    notInModule <- data.table::data.table(Entrez=setdiff(igraph::E(net)$origin, igraph::E(m1)$origin)) # ORIGIN -> GENE (gatom)
    ...
    notInModule <- data.table::data.table(Entrez=rownames(gene.exprs)) # gene.exprs2 HA!
    symbols <- AnnotationDbi::mapIds(orgdb,
                                     keys=notInModule$Entrez,
                                     column="SYMBOL",
                                     keytype="ENTREZID")
    notInModule[, symbol := unname(symbols)]
    notInModule[, cor := cor(curRev$centers.pos[i, ], t(gene.exprs[Entrez, ]))[1,]] # gene.exprs2 HA!
    notInModule <- notInModule[order(cor, decreasing=T), ]
